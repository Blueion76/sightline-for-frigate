...
SightlineCard.prototype._openInGridSlot = function(id) {
  return this._open(id);
};

SightlineCard.prototype._enter = function(...args) {
  const fromGrid=this._viewMode==='grid';
  if(fromGrid && !this._playbackReturnViewMode) this._playbackReturnViewMode='grid';
  ...
  if(engWrap){
    ...
    if(!back){
      ...
      back.type='button';
      back.id='playback-back-live';
      back.style.cssText='position:absolute;left:12px;top:12px;z-index:80;display:inline-flex;align-items:center;gap:7px;min-height:36px;padding:7px 11px;border:1px solid rgba(255,255,255,.24);border-radius:999px;background:rgba(16,16,18,.72);color:#fff;font:650 12px/1 -apple-system,BlinkMacSystemFont,system-ui,sans-serif;box-shadow:0 5px 18px rgba(0,0,0,.30);backdrop-filter:blur(16px) saturate(160%);-webkit-backdrop-filter:blur(16px) saturate(160%);cursor:pointer;appearance:none;-webkit-appearance:none';
      engWrap.appendChild(back);
    }
    const label=fromGrid?'Back to Multiview':'Back to Live';
    back.hidden=false;
    back.style.display='inline-flex';
    back.innerHTML=`${ICONS.back}<span>${label}</span>`;
    back.title=label;
    back.setAttribute('aria-label',label);
    back.onclick=()=>this._showLive();
  }
  return result;
};
...
SightlineCard.prototype._showLive = function(...args) {
  ...
  const result=baseShowLive.apply(this,args);
  this._playbackReturnViewMode=null;
  const feed=this.shadowRoot.querySelector('.workspace-feed');
  const timeline=this.shadowRoot.querySelector('.workspace-timeline');
  const media=this.shadowRoot.querySelector('.workspace-media');
  ...
  const engWrap=this.shadowRoot.querySelector('#eng-wrap');
  const grid=this.shadowRoot.querySelector('#cam-grid');
  const back=this.shadowRoot.querySelector('#playback-back-live');
...
SightlineCard.prototype._renderTimeline = function(...args) {
  ...
  const h=Math.max(48,Math.min(140,Math.round(Number(this._config?.timeline?.thumbnail_size ?? 84))));
  const w=Math.max(154,Math.min(420,Math.round(h*3.15)));
  for(const preview of this.shadowRoot?.querySelectorAll?.('.t-preview')||[]){
